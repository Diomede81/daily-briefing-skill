  ...
  try {
    ...
    const items = emails
      .filter(e => {
        ...
        // Exclude emails from Max (our own briefings)
        if (addr.includes('max@tulip-tech.com')) {
          return false;
        }
        ...
        const matchesSubject = subjectKeywords.length === 0 ||
          subjectKeywords.some(kw => subject.includes(kw.toLowerCase()));
        
        return receivedDate >= cutoff && (matchesSender || matchesSubject);
      })
      .map(e => ({
        title: e.subject,
